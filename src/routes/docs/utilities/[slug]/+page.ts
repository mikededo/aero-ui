...
const slugFromModulePath = (path: string) =>
  path.replace(Paths.utilities.content, '').replace('.md', '');
const slugFromDataPath = (path: string) =>
  path.replace(Paths.utilities.data, '').replace('.ts', '');

const getData = async (slug: string) => {
  // Cannot replace with Paths as import.meta.glob cannot analyse variables
  const modules = import.meta.glob('/src/docs/content/utilities/*.md');
  const dataModules = import.meta.glob('/src/docs/data/utilities/*.ts');
